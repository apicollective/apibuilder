...
class ServiceDaoSpec extends FlatSpec {
  ...
  private def upsertService(nameOption: Option[String] = None): ServiceDao = {
    ServiceDao.upsert(Util.createdBy, Util.testOrg, nameOption.getOrElse(name))
  }
  ...
  it should "findAll" in {
    ...
    val names = ServiceDao.findAll(ServiceQuery()).map(_.name)
  ...
  it should "find by guid" in {
    val service = upsertService()
    assertEquals(service, ServiceDao.findByGuid(service.guid).get)
  }
