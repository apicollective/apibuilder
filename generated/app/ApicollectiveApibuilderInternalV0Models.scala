 ...
 * apibuilder:0.12.38 http://localhost:9000/apicollective/apibuilder-internal/0.12.46/play_2_x_json
 */
package io.apibuilder.internal.v0.models {
   ...
   * @param guid
   * @param data
   * @param numberAttempts Records the number of times we have attempted to run this
   * task. Commonly we increment number attempts, process the task, and if succeeds
   * we then delete the task. If it fails, we update last_error. This allows us to
   * retry a task say twice; after which we no longer process the task (can notify an
   * admin of the error).
   * @param lastError
   */
  case class Task(
