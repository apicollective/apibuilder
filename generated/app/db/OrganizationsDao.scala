...
case object OrganizationsTable {
  ...
  object Columns {
    ...
    val all: List[Column] = List(Guid, Name, Key, Namespace, Visibility, CreatedAt, CreatedByGuid, UpdatedAt, UpdatedByGuid, DeletedAt, DeletedByGuid)
  }
}
...
trait BaseOrganizationsDao {
  ...
  private val parser: anorm.RowParser[Organization] = {
    anorm.SqlParser.str("guid") ~
      ...
      anorm.SqlParser.str("deleted_by_guid").? map { case guid ~ name ~ key ~ namespace ~ visibility ~ createdAt ~ createdByGuid ~ updatedAt ~ updatedByGuid ~ deletedAt ~ deletedByGuid =>
      Organization(
...
class OrganizationsDao @javax.inject.Inject() (override val db: play.api.db.Database) extends BaseOrganizationsDao {
  ...
  private val InsertQuery: io.flow.postgresql.Query = {
    io.flow.postgresql.Query("""
     ...
     | (guid, name, key, namespace, visibility, created_at, created_by_guid, updated_at, updated_by_guid)
     | values
     | ({guid}::uuid, {name}, {key}, {namespace}, {visibility}, {created_at}::timestamptz, {created_by_guid}::uuid, {updated_at}::timestamptz, {updated_by_guid}::uuid)
    """.stripMargin)
  }
