 ...
 * Service version: 0.12.46
 * apibuilder:0.12.38 http://localhost:9000/apicollective/apibuilder-api/0.12.46/play_2_4_client
 */
package io.apibuilder.api.v0.models {
  ...
  object DiffType {
    ...
    val all: List[DiffType] = List(DiffBreaking, DiffNonBreaking)
  ...
  object ItemDetailType {
    ...
    val all: List[ItemDetailType] = List(ApplicationSummary)
   ...
   * @param description
   * @param key Used as a unique key in the URL path. Key is automatically derived
   ...
   * @param name The name of the attribute. Globally unique and an 'identifier'
   * (lower case, url safe, etc.)
   * @param description Optional description - a good description here will indicate
   * which code generators it applies to and what effect it will have on those code
   * generators.
   * @param audit
   */
   ...
   * @param fromVersion
   * @param diff
   * @param changedAt Records the timestamp of when the actual change occurred (vs.
   * when we created the changed record)
   * @param audit
   ...
   * @param guid Unique identifer for this item. By using a UUID, you can combine
   * with the type to figure out the URI for the resource
   * @param detail
   * @param label
   * @param description
   */
   ...
   * @param organization
   * @param role The role this user plays for this organization. Typically member or
   * admin.
   ...
   * decline the request.
   * @param organization
   * @param role The requested role for membership to this organization. Typically
   * member or admin.
   ...
   * @param visibility Public organizations will be listed in apibuilder directory.
   * Organizations with visibility organization will only be visible to members of
   * that org.
   * @param namespace Global namespace for this organization.
   */
   ...
   * defines success
   * @param valid If true, the json schema is valid. If false, there is at least one
   * validation error.
   * @param errors Contains any validation errors that result from parsing the json
   * document. If empty, the document is valid.
   */
   ...
   * @param version The tag for this version. Can be anything, but if semver style
   * version number is used, we automatically correctly sort by version number to
   * find latest. Otherwise latest version is considered to be the most recently
   * created.
  ...
  /**
   * @param originalForm
   * @param visibility If provided, updates the visibility for all versions of this
   * application
   */
  ...
  object OriginalType {
     ...
     * above.
     */
    val all: List[OriginalType] = List(ApiJson, AvroIdl, ServiceJson, Swagger)
  ...
  object Publication {
     ...
     * above.
     */
    val all: List[Publication] = List(MembershipRequestsCreate, MembershipsCreate, ApplicationsCreate, VersionsCreate)
  ...
  object Visibility {
     ...
     * above.
     */
    val all: List[Visibility] = List(User, Organization, Public)
...
package io.apibuilder.api.v0 {
  ...
  object Constants {
    ...
    val UserAgent = "apibuilder:0.12.38 http://localhost:9000/apicollective/apibuilder-api/0.12.46/play_2_4_client"
    val Version = "0.12.46"
    val VersionMajor = 0
    ...
    object Applications extends Applications {
      override def get(
        ...
        _executeRequest("GET", s"/${play.utils.UriEncoding.encodePathSegment(orgKey, "UTF-8")}", queryParameters = queryParameters, requestHeaders = requestHeaders).map {
          ...
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200")
        }
      }
      ...
      override def post(
        ...
        _executeRequest("POST", s"/${play.utils.UriEncoding.encodePathSegment(orgKey, "UTF-8")}", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200, 409")
        }
      }
      ...
      override def deleteByApplicationKey(
        ...
        _executeRequest("DELETE", s"/${play.utils.UriEncoding.encodePathSegment(orgKey, "UTF-8")}/${play.utils.UriEncoding.encodePathSegment(applicationKey, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 204")
        }
      }
    ...
    object Attributes extends Attributes {
      ...
      override def getByName(
        ...
        _executeRequest("GET", s"/attributes/${play.utils.UriEncoding.encodePathSegment(name, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 404 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200, 404")
        }
      }
      ...
      override def post(
        ...
        _executeRequest("POST", s"/attributes", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 401 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 201, 401, 409")
        }
      }
      ...
      override def deleteByName(
        ...
        _executeRequest("DELETE", s"/attributes/${play.utils.UriEncoding.encodePathSegment(name, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 401 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r if r.status == 404 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 204, 401, 404")
        }
      }
    }
    ...
    object Code extends Code {
      override def get(
        ...
        _executeRequest("GET", s"/${play.utils.UriEncoding.encodePathSegment(orgKey, "UTF-8")}/${play.utils.UriEncoding.encodePathSegment(applicationKey, "UTF-8")}/${play.utils.UriEncoding.encodePathSegment(version, "UTF-8")}/${play.utils.UriEncoding.encodePathSegment(generatorKey, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 404 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200, 404, 409")
        }
      }
    }
    ...
    object EmailVerificationConfirmationForms extends EmailVerificationConfirmationForms {
      override def post(
        ...
        _executeRequest("POST", s"/email_verification_confirmations", body = Some(payload), requestHeaders = requestHeaders).map {
          case r if r.status == 204 => ()
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 204, 409")
        }
      }
    }
    ...
    object GeneratorServices extends GeneratorServices {
      ...
      override def deleteByGuid(
        ...
        _executeRequest("DELETE", s"/generator_services/${guid}", requestHeaders = requestHeaders).map {
          case r if r.status == 204 => ()
          case r if r.status == 403 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r if r.status == 404 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 204, 403, 404")
        }
      }
    }
    ...
    object Organizations extends Organizations {
      ...
      override def putAttributesByKeyAndName(
        ...
        _executeRequest("PUT", s"/organizations/${play.utils.UriEncoding.encodePathSegment(key, "UTF-8")}/attributes/${play.utils.UriEncoding.encodePathSegment(name, "UTF-8")}", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 404 => throw new io.apibuilder.api.v0.errors.UnitResponse(r.status)
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 200, 201, 404, 409")
        }
      }
    ...
    object Subscriptions extends Subscriptions {
      ...
      override def post(
        ...
        _executeRequest("POST", s"/subscriptions", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 409 => throw new io.apibuilder.api.v0.errors.ErrorsResponse(r)
          case r => throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 201, 409")
        }
      }
    ...
    def parseJson[T](
      ...
      f(play.api.libs.json.Json.parse(r.body)) match {
        ...
        case play.api.libs.json.JsError(errors) => {
          throw new io.apibuilder.api.v0.errors.FailedRequest(r.status, s"Invalid json for class[" + className + "]: " + errors.mkString(" "))
        }
      }
    }
  ...
  trait Applications {
     ...
     * @param hasVersion If true, we return applications that have at least one
     * version. If false, we return applications that have no versions in the system
  ...
  trait Changes {
     ...
     * @param applicationKey Filter changes to those made for the application with this
     * key.
  ...
  trait Items {
    /**
     * @param q The search query. At the moment, we do case insensitive exact match
     * search.
     * @param limit The number of records to return
     * @param offset Used to paginate. First page of results is 0.
     */
  ...
  trait Organizations {
     ...
     * @param userGuid If specified, restricts to organizations that this user is
     * specifically a member of (e.g. will exclude public organizations with which the
     * user does not have a direct membership).
     ...
     * @param namespace Find organizations with this namespace. Case in-sensitive.
     * Exact match
     * @param limit The number of records to return
     * @param offset Used to paginate. First page of results is 0.
     */
  ...
  trait Versions {
     ...
     * @param version The version of tthis application to download, or the keyword
     * latest to get the latest version
     */
     ...
     * @param version The version of tthis application to download, or the keyword
     * latest to get the latest version
     * @param typeName The name of the type (e.g. model name) for which you would like
     * to generate a sample json document
     * @param optionalFields If true, we generate sample data for all optional fields.
     * Otherwise the generated sample will contain only required fields, where
     * applicable
     */
  ...
  trait Watches {
     ...
     * @param userGuid The user for which we are checking. API returns false if the
     * user guid is not provided
