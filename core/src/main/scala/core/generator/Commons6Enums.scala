...
object Commons6Enums {

  def build(enum: ScalaEnum): String = {
    Seq(
      enum.description.map { desc => ScalaUtil.textToComment(desc) + "\n" }.getOrElse("") +
        s"sealed abstract class ${enum.name} {",
      s"@JsonValue override def toString: String".indent(2),
      s"}",
      s"object ${enum.name} {",
...
""" +
    ...
    s"@JsonCreator def apply(value: String): ${enum.name} = fromString(value).getOrElse(UNDEFINED(value))\n\n" +
