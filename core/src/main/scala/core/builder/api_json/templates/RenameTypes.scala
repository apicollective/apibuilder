...
case class RenameTypes(casts: Seq[TemplateTypeCast]) {
  ...
  private val byTypeName: Map[String, Seq[TemplateTypeCast]] = casts.groupBy(_.typeName)
  ...
  private def renameInterface(context: String, interface: Interface): Interface = {
    interface.copy(
      fields = interface.fields.map(_.map(renameField(context, _)))
    )
  }

  private def renameResource(context: String, res: Resource): Resource = {
    res.copy(
      operations = res.operations.map(renameOperation(context, _))
    )
  }

  private def renameOperation(context: String, op: Operation): Operation = {
    op.copy(
  ...
  private def renameModel(context: String, m: Model): Model = {
    m.copy(
      fields = m.fields.map(renameField(context, _))
    )
  }

  private def renameBody(context: String, body: Body): Body = {
    body.copy(
      `type` = renameType(context, body.`type`)
    )
  }

  private def renameResponse(context: String, response: Response): Response = {
    response.copy(
      `type` = renameType(context, response.`type`)
    )
  }
  private def renameParameter(context: String, param: Parameter): Parameter = {
    param.copy(
  ...
  private def renameField(context: String, field: Field): Field = {
    field.copy(
      `type` = renameType(context, field.`type`)
    )
  }

  private def renameType(context: String, typ: String): String = {
    TextDatatype.label(
