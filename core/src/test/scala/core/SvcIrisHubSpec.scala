...
class SvcIrisHubSpec extends FunSpec with Matchers {
  ...
  it("parses models") {
    ...
    item.fields.find(_.name == "number").get.`type` should be(TypeInstance(Container.Singleton, Type.Primitive(Primitives.String)))
  }
  ...
  it("parses operations") {
    ...
    response.`type` should be(TypeInstance(Container.List, Type.Model("item")))
