...
import io.apibuilder.apidoc.api.v0.{Authorization, Client}
...
object ApiClient {
  ...
  def callWith404[T](
    ...
    f.map {
      ...
      case io.apibuilder.apidoc.api.v0.errors.UnitResponse(404) => None
...
case class ApiClient(sessionId: Option[String]) {
  ...
  val client: Client = new io.apibuilder.apidoc.api.v0.Client(
    baseUrl = baseUrl,
    auth = Some(apiAuth),
