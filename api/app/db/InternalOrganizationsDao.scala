...
class InternalOrganizationsDao @Inject()(
      ...
      findAll(Authorization.All, name = Some(form.name), limit = Some(1)).headOption match {
        ...
        case Some(org) => {
          if (existing.map(_.guid).contains(org.guid)) {
            ...
            Seq(s"Org with the name '${form.name}' already exists")
          }
        }
      }
    }
            ...
            findByKey(Authorization.All, key) match {
              ...
              case Some(found) => {
                if (existing.map(_.guid).contains(found.guid)) {
                  ...
                  Seq(s"Org with the key '${found.key}' already exists")
                }
              }
            }
          }
  ...
  def findAllByGuids(authorization: Authorization, guids: Seq[UUID]): Seq[InternalOrganization] = {
  ...
  def findAll(
    ...
    dao.findAll(
      ...
      .equals("lower(namespace)", namespace.map(_.toLowerCase().trim))
      .and(isDeleted.map {
        case false => "deleted_at is null"
        case true => "deleted_at is not null"
      })
      .and(
        userGuid.map { _ =>
