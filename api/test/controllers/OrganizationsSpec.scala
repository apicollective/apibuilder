...
class OrganizationsSpec extends BaseSpec {
  ...
  "POST /organizations" in new WithServer {
    ...
    val org = createOrganization(OrganizationForm(key = key))
    org.key must be(key)
  }

  "POST /organizations trims whitespace in key" in new WithServer {
    val key = UUID.randomUUID.toString
    val org = createOrganization(OrganizationForm(key = " " + key + " "))
    org.key must be(key)
  }
  ...
  "POST /organizations validates key is not reserved" in new WithServer {
    intercept[ErrorsResponse] {
      createOrganization(OrganizationForm(key = "user"))
    }.errors.map(_.message) must be(Seq(s"Key user is a reserved word and cannot be used for the name of an organization"))
  }
