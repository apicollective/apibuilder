...
class ServicesDaoSpec extends PlaySpec with OneAppPerSuite with db.Helpers {
  ...
  "validate" in {
    ...
    val service = servicesDao.create(db.createdBy, form)
  ...
  "softDelete" in {
    val service = servicesDao.create(db.createdBy, createGeneratorServiceForm())
    servicesDao.softDelete(db.createdBy, service)
