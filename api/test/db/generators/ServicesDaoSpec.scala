...
class ServicesDaoSpec extends PlaySpec with OneAppPerSuite with Helpers {
  ...
  "create" must {
    ...
    "creates a service" in {
      val form = createGeneratorServiceForm()
      val service = servicesDao.create(createdBy, form)
  ...
  "softDelete" in {
    val service = servicesDao.create(createdBy, createGeneratorServiceForm())
    servicesDao.softDelete(createdBy, service)
